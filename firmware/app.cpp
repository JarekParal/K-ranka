...
void destroyEnemy(ev3cxx::StopWatch stopWatch, int time, Robot& robot, KetchupLogic* controller){
	if (stopWatch.getMs() > time) {

		controller->go({0, 5});
		controller->face(Pred::East);

		robot.findLine();
		robot.step(6, Robot::Debug::Default,  true);
	}
}
...
//     RobotGeometry robotGeometry{55, 128};
	RobotGeometry robotGeometry{ 55, 130 };
	...
	auto robot = std::make_unique < Robot >( robotGeometry, colorL, colorR, ketchupSensor, btnEnter, btnStop, motors,
	                                         motorGate,
	                                         l, bt, sonar, motorSensor,
	                                         Robot::Debug( Robot::Debug::Text | Robot::Debug::Packet ) );
...
//	controller->face(Pred::East);
//	ev3cxx::delayMs( 2000 );
//	controller->face(Pred::West);
//	ev3cxx::delayMs( 2000 );



	ev3cxx::StopWatch stopWatch {};
	int time = 30000;
...
//	controller->go( { 4, 0 } );
//	destroyEnemy(stopWatch, time, *robot, controller);
